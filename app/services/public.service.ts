...
export class PublicService {
	...
	// 时间选择
	public timeRangePickerSet(id,options,handler,hideHandler?){
		$("#" + id).daterangepicker(options,handler);
		hideHandler && $("#" + id).on('hide.daterangepicker',hideHandler);  
	}
	// 截取文件名
    public cutFileName(str){
        let arr = str.split("\\");
        let fileName = arr[arr.length-1];
        return fileName;
    }
	// 数组筛选元素
	public isExistByArr(id,arr){
		if(!arr){
			return false;
		}
	...
	// 数组对象筛选元素
	public isExistByArrObj(id,arr,name){
		if(!arr){
			return false;
		}
		for(let i = 0;i < arr.length;i++){
	...
	// 数组对象筛选元素
	public isExistByArrObj2(id,arr,name,adxType,name2){
		if(!arr){
			return false;
		}
		for(let i = 0;i < arr.length;i++){
			if(arr[i][name] == id && arr[i][name2] == adxType){
				return true
			}
		}
		return false;
	...
	// 数组对象返回筛选后元素
	public returnExistByArrObj(nameArr,arr,name){
		if(!nameArr || !arr){
			return [];
		}
		let newArr = [];
	...
	// 补齐长度
    public addLength(str){
        let addStr = "";
        for(let i = 0;i < 24 - str.length;i++){
            addStr += "0";
        }
        str = addStr + str;
        return str;
	...
	// 去前后空格
	public removeBlank(str){
		return str.replace(/(^\s*)|(\s*$)/g, "");
	}
	// 回车替换成逗号
	public blankToPoint(str){
		return str.replace(/\n/g,",");
	}
	// 中文逗号换英文逗号
	public transformPoint(str){
		return str.replace(/，/g,",");
	}
	// 判断字符长度
	public getLength(str){
		let maxlength = 0;
